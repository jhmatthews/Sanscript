...
#nobs=int(raw_input('enter n_obs:'))

pf_filename=sys.argv[1]+".pf"
inp =open(pf_filename,'r')

for line in inp.readlines():
		data=line.split()
		print data[0]
		if (data[0]=="lum_agn(ergs/s)"): lpl=float(data[1])
		elif (data[0]=="agn_power_law_index"): alpha=float(data[1])
		elif (data[0]=="rstar(cm)"): 	
			rbb=float(data[1])
			disk_radmin=float(data[1])	
		elif (data[0]=="tstar"): tbb=float(data[1])
		elif (data[0]=="Star_radiation(y=1)"): istar=int(data[1])
		elif (data[0]=="Disk_radiation(y=1)"): idisk=int(data[1])
		elif (data[0]=="QSO_BH_radiation(y=1)"): ibh=int(data[1])
		elif (data[0]=="disk.mdot(msol/yr)"): disk_mdot=float(data[1])
		elif (data[0]=="disk.radmax(cm)"): disk_radmax=float(data[1])
		elif (data[0]=="mstar(msol)"): mbh=float(data[1])		
		elif (data[0]=="no_observers"): nobs=int(data[1])
		elif (data[0]=="spectrum_wavemin"): lmin=float(data[1])		
		elif (data[0]=="spectrum_wavemax"): lmax=float(data[1])
...
if yno=='y':
	for i in range(nobs):
		filename=orig_filename+'_'+str(i+1)

...
for i in range(len(sys.argv)):
	if sys.argv[i]=='-s': 
		sm_mode=True
		sm_commands=sys.argv[i+1]
	if sys.argv[i]=='-n': norm_mode=True
	if sys.argv[i]=='-r': 
		range_mode=True
		lmin=float(sys.argv[i+1])
		lmax=float(sys.argv[i+2])
	if sys.argv[i]=='-sc': 
		comp_mode=True
		sm_mode=True
	if sys.argv[i]=='-snc': 
...
def help():
	help_string=''' You want help...ok...
		        usage genplot filename [ibin] [-n] [-s] [-r lmin lmax]
 		        -n means normalised
			-s means SM mode
...
if range_mode: 
	...
	plt.savefig(sys.argv[1]+"_range"+str(lmin)+"_"+str(lmax)+'_'+save_suffix+'summary.jpg',dpi=80,facecolor='w',edgecolor='w',orientation='portrait')

else:
	#save spectrum summary figure
	plt.savefig(sys.argv[1]+'_'+save_suffix+'summary.jpg',dpi=80,facecolor='w',edgecolor='w',orientation='portrait')
...
if Sources:
	...
	if range_mode:
		...
		plt.savefig(sys.argv[1]+"_range"+sys.argv[4]+"_"+sys.argv[5]+'_'+save_suffix+'summary.jpg',dpi=80,facecolor='w',edgecolor='w',orientation='portrait')

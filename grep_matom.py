...
#do an ls in folder to ggedet all the c files.
...
for i_c_file in range(len(all_c_files)):
	...
	if all_c_files[i_c_file]=='matom.c': ran_y=0.0
...
print c_called_by
for i_c_file in range(len(c_files)):
	print i_c_file
		...
		while j<len(all_c_files) and found==0:
			for k in range(len(c_functions_in[j])):
				if c_called_by[i_c_file][i]==c_functions_in[j][k]:
					...
					plt.text(x,y,c_called_by[i_c_file][i],fontsize=4)

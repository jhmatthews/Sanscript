...
# syntax python spectrum_onepage_list.py files.ls 20 1480 1580 1

list_filename=sys.argv[1]
if sys.argv[1]=='help': print 'syntax speclist files.ls 20 1480 1580 1'

# set binning smoothing
if (len(sys.argv)<3):
	ibin=1
else:
	ibin=int(sys.argv[2])
...
for line in inp_list:
	i_file=i_file+1
	if i_file>file_length: 'THERE IS AN ERROR HERE!'
	print 'CARRYING OUT SPECTRA PLOT: Number '+str(i_file)+' out of '+str(file_length)
	file_arr=line.split()
	fname=str(file_arr[0])
	...
	# set normalisation and minimum and max wavelengths
	if (len(sys.argv)==5):
	...
	if (len(sys.argv)>3): 
		...
		plt.savefig(fname+"_range"+sys.argv[3]+"_"+sys.argv[4]+'spectrum_summary.jpg',dpi=80,facecolor='w',edgecolor='w',orientation='portrait')

	else:
		#save spectrum summary figure
		plt.savefig(fname+'spectrum_summary.jpg',dpi=80,facecolor='w',edgecolor='w',orientation='portrait')
	...
	if (len(sys.argv)>3):
		...
		plt.savefig(fname+"_range"+sys.argv[3]+"_"+sys.argv[4]+'spectrum_sources.jpg',dpi=80,facecolor='w',edgecolor='w',orientation='portrait')

	else:
			#save spectrum sources figure
		plt.savefig(fname+'spectrum_sources.jpg',dpi=80,facecolor='w',edgecolor='w',orientation='portrait')

	


#os.system('rm -f '+list_filename)
print 'ALL DONE, '+str(i_file)+' SUMMARY PLOTS'
